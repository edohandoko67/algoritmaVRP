...
class MainActivity4 : AppCompatActivity(), OnMapReadyCallback, GoogleMap.OnMarkerClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnNearestNeighbor = findViewById(R.id.btn_nearest_neighbor2)
        textJarakContent = findViewById(R.id.text_jarak_content2)
        locationList = ArrayList()

        database.child("db2").addValueEventListener(object : ValueEventListener {
...
//              Mengubah data dari Firebase menjadi list
                ...
                nearest.removeLast();
                nearest.removeFirst();
                mMap2.animateCamera(CameraUpdateFactory.newLatLngZoom(originLocation, 13F))


                btnNearestNeighbor.setOnClickListener {
                    val urll = getDirectionURL(originLocation, nearest, destinationLocation, apiKey)
                    GetDirection(urll).execute()
                    mMap2.animateCamera(CameraUpdateFactory.newLatLngZoom(originLocation, 14F))
            ...
            fusedLocationClient.lastLocation
                .addOnSuccessListener { location: Location? ->
                    ...
                    location?.let {
                        ...
                        mMap2.moveCamera(CameraUpdateFactory.newLatLngZoom(currentLatLng, 15f))
                    }
                }
        } else {
